...
function(get_arduino_flags COMPILE_FLAGS_VAR LINK_FLAGS_VAR BOARD_ID MANUAL)
    ...
    # Check for platform:core variety
    if("${BOARD_CORE}" MATCHES ":")
      arduino_debug_msg("BOARD_CORE contains ':' ${BOARD_CORE}, extracting core (ignoring platform)")
      string(REGEX REPLACE "^[^:]+:" "" BOARD_CORE_NEW ${BOARD_CORE})
      set(BOARD_CORE ${BOARD_CORE_NEW})
    endif()

    set(BOARD_CORE_VARIANT ${${BOARD_ID}.build.variant})
    if("${BOARD_CORE_VARIANT}" MATCHES ":")
      arduino_debug_msg("BOARD_CORE_VARIANT contains ':' ${BOARD_CORE_VARIANT}, extracting variant (ignoring platform)")
      string(REGEX REPLACE "^[^:]+:" "" BOARD_CORE_VARIANT_NEW ${BOARD_CORE_VARIANT})
      set(BOARD_CORE_VARIANT ${BOARD_CORE_VARIANT_NEW})
    endif()
    ...
    if(BOARD_CORE)
        ...
        if(ARDUINO_SDK_VERSION VERSION_GREATER 1.0 OR ARDUINO_SDK_VERSION VERSION_EQUAL 1.0)
            if(NOT MANUAL)
                set(PIN_HEADER ${${BOARD_CORE_VARIANT}.path})
                if(PIN_HEADER)
...
function(setup_arduino_core VAR_NAME BOARD_ID)
    ...
    # Check for platform:core variety
    if("${BOARD_CORE}" MATCHES ":")
      arduino_debug_msg("Arduino board_core contains ':' ${BOARD_CORE}")
      string(REGEX REPLACE "^[^:]+:" "" BOARD_CORE_NEW ${BOARD_CORE})
      arduino_debug_msg("${BOARD_CORE} -> ${BOARD_CORE_NEW}")
      set(BOARD_CORE ${BOARD_CORE_NEW})
    endif()
    if(BOARD_CORE)
        if(NOT TARGET ${CORE_LIB_NAME})
            set(BOARD_CORE_PATH ${${BOARD_CORE}.path})
            arduino_debug_msg("Arduino board_core.path: ${arduino.arduino.path}")
            arduino_debug_msg("Arduino board_core: ${BOARD_CORE}")
            arduino_debug_msg("Arduino coard_core_path: ${BOARD_CORE_PATH}")
